...
CREATE TABLE `member` (
  ...
  `member_manager` tinyint(1) NOT NULL DEFAULT '0',
...
-- muluck.inquiry definition

CREATE TABLE `inquiry` (
  `inquiry_no` int NOT NULL AUTO_INCREMENT,
  `inquiry_writer` int NOT NULL,
  `inquiry_title` varchar(100) NOT NULL,
  `inquiry_content` varchar(300) NOT NULL,
  `inquiry_cdate` date NOT NULL,
  `inquiry_udate` date NOT NULL,
  `inquiry_manager` int DEFAULT NULL,
  `inquiry_answer` varchar(300) DEFAULT NULL,
  `inquiry_adate` date DEFAULT NULL,
  PRIMARY KEY (`inquiry_no`),
  KEY `inquiry_FK` (`inquiry_writer`),
  KEY `inquiry_FK_1` (`inquiry_manager`),
  CONSTRAINT `inquiry_FK` FOREIGN KEY (`inquiry_writer`) REFERENCES `member` (`member_no`),
  CONSTRAINT `inquiry_FK_1` FOREIGN KEY (`inquiry_manager`) REFERENCES `member` (`member_no`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;